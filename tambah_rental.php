...
<title>UAS PI : Tambah Rental</title>
...
<div class="header">
<div class="pull-left">
<div class="logo"><a href="tambah_rental.php"><img src="assets/images/(AM) Mana Void Build.png" width="10%" alt="" />
    ...
    <div class="page-header">
        <div class="page-title">
            <ol class="breadcrumb text-right">
                <li><a href="tambah_rental.php">Dashboard</a></li>
            </ol>
        </div>
    </div>
</div>
                ...
                <div class="basic-form">
                    <form method="POST" action="fungsi_tambah_rental.php">
                        <div class="form-group">
                            ...
                            <input type="text" name="no_trx_anggaaf" id="no_trx_anggaaf"
                                class="form-control" placeholder="Masukkan NIK"
                                value="<?php echo $trx; ?>" readonly required>
                        </div>
                </div>
                <div class="form-group">
                    <label for="nik_ktp_anggaaf" class="form-label">Pilih Pelanggan</label>
                    <select class="form-control" name="nik_ktp_anggaaf" id="nik_ktp_anggaaf" required>
                        <option value=""> -- Pilih Pelanggan --</option>
                        <?php
                        include "config/koneksi.php";
                        $tampil = mysqli_query($koneksi, "SELECT * FROM tbl_pelanggan_anggaaf");
                        while ($data = mysqli_fetch_array($tampil)) {
                            echo "<option value='$data[nik_ktp_anggaaf]'>$data[nik_ktp_anggaaf] - $data[nama_anggaaf] </option>";
                        }
                        ?>
                    </select>
                </div>
                <div class="form-group">
                    <label for="no_plat_anggaaf" class="form-label">Pilih Mobil</label>
                    <select class="form-control" name="no_plat_anggaaf" id="no_plat_anggaaf" required>
                        <option value=""> -- Pilih Mobil --</option>
                        <?php
                        include "config/koneksi.php";
                        $tampil = mysqli_query($koneksi, "SELECT * FROM tbl_mobil_anggaaf");
                        while ($data = mysqli_fetch_array($tampil)) {
                            echo "<option value='$data[no_plat_anggaaf]'>$data[no_plat_anggaaf] - $data[nama_mobil_anggaaf] </option>";
                        }
                        ?>
                    </select>
                </div>
                <div class="form-group">
                    <label for="tgl_rental_anggaaf" class="form-label">Tanggal Ambil</label>
                    <input type="date" name="tgl_rental_anggaaf" id="tgl_rental_anggaaf"
                        class="form-control" required>
                </div>
                <div class="form-group">
                    <label for="jam_rental_anggaaf" class="form-label">Jam Ambil</label>
                    <input type="time" name="jam_rental_anggaaf" id="jam_rental_anggaaf"
                        class="form-control" required>
                </div>
                <div class="form-group">
                    <label for="lama_anggaaf" class="form-label">Lama Rental (per Hari)</label>
                    <input type="number" name="lama_anggaaf" id="lama_anggaaf" class="form-control"
                        oninput="hitungTotalBayar()" required>
                </div>
                <div class="form-group">
                    <label for="harga_anggaaf" class="form-label">Harga Rental</label>
                    <input type="text" name="harga_anggaaf" id="harga_anggaaf"
                        class="form-control money" oninput="hitungTotalBayar()" required>
                </div>
                <div class="form-group">
                    <label for="total_anggaaf" class="form-label">Total Bayar</label>
                    <input type="text" name="total_anggaaf" id="total_anggaaf" readonly
